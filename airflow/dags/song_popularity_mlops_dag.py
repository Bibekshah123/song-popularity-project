...
with DAG(
    ...
    description="MLOps pipeline — MariaDB + Redis — monitoring is lifecycle activity",
    ...
    data_ingestion = BashOperator(
        ...
        bash_command=f"cd {PROJECT_DIR} && python src/etl_ingestion.py",
    )

    data_validation = BashOperator(
        task_id="data_validation",
        bash_command=f"cd {PROJECT_DIR} && python src/validate_data.py",
    )
    ...
    model_deployment = BashOperator(
        ...
        bash_command=f"cd {PROJECT_DIR} && docker compose up -d --build backend",
    )
